#!/usr/bin/env python3
"""run_inference_fig7_conf.py — same as run_inference_fig7.py but with a
confidence threshold on 'a' classifications.

If the softmax probability for 'a' is below CONF_THRESHOLD, the segment is
classified as the runner-up class instead.  This prevents low-confidence 'a'
predictions from prematurely starting (or extending) a repeat run.

Writes predicted .not.mat files to:
  /Users/riekers/.moove/rec_data/gy07bu07/fig7_pred_conf/250902/

Run compare_repeat_targeting_conf.py afterwards to evaluate.
"""
...
OUT           = Path(__file__).parent / "inference_fig7_results_conf.json"
PRED_CONF_DIR = Path("/Users/riekers/.moove/rec_data/gy07bu07/fig7_pred_conf/250902")
...
CONF_THRESHOLD = 0.7  # min softmax p('a') to accept 'a'; below → use runner-up
...
def run_cls_inference(audio, onset_ms, cls_model, cls_meta, device):
    ...
    # Confidence gate: if top class is 'a' but below threshold → use runner-up
    if pred_label == 'a' and probs[pred_idx] < CONF_THRESHOLD:
        sorted_idx = np.argsort(probs)[::-1]
        pred_label = int_to_label[sorted_idx[1]]
    return pred_label, int_to_label
...
def main():
    ...
    for fid in TARGET_FILES:
        ...
        pred_labeled = []  # (onset_frame, onset_ms, pred_label, offset_frame, offset_ms)
        for p_on, p_off in pred_segs:
            p_onset_ms  = p_on  * MS_PER_FRAME
            p_offset_ms = p_off * MS_PER_FRAME
            plabel, _ = run_cls_inference(audio, p_onset_ms, cls_model, cls_meta, device)
            if plabel is not None:
                pred_labeled.append((p_on, p_onset_ms, plabel, p_off, p_offset_ms))

        # Write predicted .not.mat to fig7_pred_conf/
        PRED_CONF_DIR.mkdir(parents=True, exist_ok=True)
        notmat_out = PRED_CONF_DIR / (wav_path.name + ".not.mat")
        pred_onsets_ms_arr  = np.array([x[1] for x in pred_labeled], dtype=np.float64).reshape(1, -1)
        pred_offsets_ms_arr = np.array([x[4] for x in pred_labeled], dtype=np.float64).reshape(1, -1)
        pred_labels_str_out = "".join(x[2] for x in pred_labeled)
        savemat(str(notmat_out), {
            "Fs":        np.array([[float(SAMPLE_RATE)]]),
            "fname":     wav_path.name,
            "labels":    pred_labels_str_out,
            "onsets":    pred_onsets_ms_arr,
            "offsets":   pred_offsets_ms_arr,
            "min_int":   np.array([[0.0]]),
            "min_dur":   np.array([[0.0]]),
            "threshold": np.array([[float(seg_meta["tuned_threshold"])]]),
            "sm_win":    np.array([[0.0]]),
        })

...
Reads results.json from paper_experiments/results/seg/ (5 birds x 3 seeds)
and produces a multi-panel figure:
  ...
  C) Segment-level collar F1 vs tolerance per bird (WITH sliding window)
  D) Training set duration vs Framewise F1 (scatter)

Outputs are saved as SVG and PNG in new_plots/figure_4/.
"""
import json
import os
import sys
...
def load_seg_results():
    ...
    for bird in BIRD_IDS:
        runs = []
        for seed in SEEDS:
            path = os.path.join(RESULTS_DIR, "seg", bird, f"seed_{seed}", "results.json")
...
def plot_framewise_dots(ax, results):
    ...
    ax.set_xlabel("Score")
    ax.set_title("Framewise Segmentation Metrics")
...
def plot_collar_f1(ax, results):
    """Onset-based segment-level F1 vs collar tolerance (with sliding window)."""
    for bird in BIRD_IDS:
        if bird not in results:
            continue
        runs = results[bird]
    ...
    ax.set_title("Onset Collar F1 (with sliding window)")
...
def print_summary_table(results):
    """Print combined summary (RAW framewise + SW collar)."""
    print("\n=== SEGMENTATION SUMMARY (RAW framewise + SW collar) ===")
    header = (f"{'Bird':<10} {'Label':<8} {'Dur(s)':<10} {'Syl_train':<10} "
              f"{'Epochs':<8} {'FW-P':<18} {'FW-R':<18} {'FW-F1':<18} "
              f"{'Col@10ms(SW)':<18} {'Col@20ms(SW)':<18}")
    print(header)
    print("-" * len(header))
    ...
    for bird in BIRD_IDS:
        ...
        c10 = [r["onset_collar_smoothed"]["@10ms"]["f1"] for r in runs]
        c20 = [r["onset_collar_smoothed"]["@20ms"]["f1"] for r in runs]

        def ms(vals):
            a = np.array(vals)
            return f"{a.mean():.4f} +/- {a.std():.4f}"

        print(f"{bird:<10} {label:<8} {np.mean(durs):<10.1f} {int(np.mean(syls)):<10} "
              f"{np.mean(epochs):<8.1f} {ms(fw_p):<18} {ms(fw_r):<18} "
              f"{ms(fw_f1):<18} {ms(c10):<18} {ms(c20):<18}")
